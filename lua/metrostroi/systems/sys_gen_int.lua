...
function TRAIN_SYSTEM.SolveEzh3(Train,Triggers)
	...
	S["2Ye"] = Train.LK4.Value*S["2Zh-2A"]*Train.A2.Value*(1-2*Train.RRP.Value)*((1-Train.RRP.Value) + Train.RRP.Value*Train.A39.Value)*S["2A-2G"]*Train:ReadTrainWire(2)+T[5]*(1.0-Train.LK4.Value)
...
function TRAIN_SYSTEM.Solve81_717(Train,Triggers)
	...
	S["6Yu"] = S["6A"]*C((P == 3) or (P == 4))*C((RK >= 1) and (RK <= 5))
	S["2Ye"] = Train.LK4.Value*S["2Zh-2A"]*Train.A2.Value*(1-2*Train.RRP.Value)*((1-Train.RRP.Value) + Train.RRP.Value*Train.A39.Value)*S["2A-2G"]*Train:ReadTrainWire(2)+T[15]*(1.0-Train.LK4.Value)
	S["B2"] = 1*Train.VB.Value*B
	S["33V"] = S["10AK"]*S["10AK-VAH1"]*1*Train.RV2.Value*S["33B-33AVU"]
	S["18A"] = Train.A14.Value*(1.0-Train.RPvozvrat.Value)*Train.LK4.Value*1
	S["B8"] = S["B2"]*Train.A53.Value
...
function TRAIN_SYSTEM.Solve81_714(Train,Triggers)
	...
	S["1R-1A"] = C((P == 2))*S["1A-1M"]+(1.0-Train.RV1.Value)*C((P == 1))
	S["10AG-10E"] = S["10AD-10AG"]*S["10AP-10AD"]*(1.0-Train.LK1.Value)+C((P == 1))*Train.LK3.Value*C(RK == 18)
	S["1E-1Yu"] = Train.KSH2.Value+Train.KSB2.Value*Train.KSB1.Value
	S["1G-1Zh"] = Train.LK3.Value+Train.LK5.Value*C(RK == 1)*C((P == 1) or (P == 3))*S["1E-1Yu"]
	S["2A-2G"] = C((P == 1) or (P == 3))*C((RK >= 1) and (RK <= 17))+C((P == 2) or (P == 4))*S["2V-2G"]
	S["10"] = 1*Train:ReadTrainWire(10)
	S["2Ye"] = S["2A-2G"]*Train.A2.Value*(1-2*Train.RRP.Value)*((1-Train.RRP.Value) + Train.RRP.Value*Train.A39.Value)*S["2Zh-2A"]*Train.LK4.Value*Train:ReadTrainWire(2)+T[27]*(1.0-Train.LK4.Value)
	S["4B"] = (1.0-Train.RKR.Value)*Train:ReadTrainWire(4)
	S["5B"] = Train.RKR.Value*Train:ReadTrainWire(5)
