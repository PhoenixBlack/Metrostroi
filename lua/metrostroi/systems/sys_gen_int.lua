...
function TRAIN_SYSTEM.SolveEzh3(Train,Triggers)
	...
	S["10E-10AG"] = (1.0-Train.LK1.Value)*S["10AP-10AD"]*S["10AG-10AD"]+Train.LK3.Value*C(RK == 18)*C((P == 1))
	...
	S["33V"] = S["10AK"]*Train.RV2.Value*S["10AK-VAH1"]*S["33B-33AVU"]*1
	...
	S["2Ye"] = Train.LK4.Value*Train.A2.Value*S["2Zh-2A"]*(1-2*Train.RRP.Value)*((1-Train.RRP.Value) + Train.RRP.Value*Train.A39.Value)*S["2A-2G"]*Train:ReadTrainWire(2)+T[5]*(1.0-Train.LK4.Value)
...
function TRAIN_SYSTEM.Solve81_717(Train,Triggers)
	...
	S["33Aa"] = S["10AS"]*Train.KV["10AS-33"]*S["33-33Aa"]
	S["16V"] = Train.A16.Value*(1.0-Train.RD.Value)*Train:ReadTrainWire(16)
	S["1P"] = S["1A"]*S["1T-1P"]*C((P == 1) or (P == 2))+T[12]*C((P == 3) or (P == 4))
	S["33V"] = S["10AK"]*Train.RV2.Value*S["10AK-VAH1"]*S["33B-33AVU"]*1
	S["B2"] = 1*Train.VB.Value*B
	S["10AN"] = 1*(1.0-Train.RPvozvrat.Value)*Train.A14.Value*1
	S["8"] = S["10"]*S["10-8"]+Train.ALS_ARS["8"]
	...
	S["1R"] = S["1A"]*S["1A-1R"]
	S["1"] = S["10AS"]*Train.R1_5.Value*Train.KV["10AS-33D"]*Train.ALS_ARS["33D"]+(-10*Train.KRU["1/3-ZM31"])
	S["2Ye"] = Train.LK4.Value*Train.A2.Value*S["2Zh-2A"]*(1-2*Train.RRP.Value)*((1-Train.RRP.Value) + Train.RRP.Value*Train.A39.Value)*S["2A-2G"]*Train:ReadTrainWire(2)+T[15]*(1.0-Train.LK4.Value)
	S["1Zh"] = S["1P"]*S["1G-1Zh"]*Train.AVT.Value*(1.0-Train.RPvozvrat.Value)
	S["B8"] = S["B2"]*Train.A53.Value
	S["B22"] = S["B8"]*Train.A75.Value
	S["B28"] = S["B8"]*Train.KUP.Value
	...
	S["D1"] = S["10/4"]*Train.A21.Value*Train.KV["D-D1"]+(1*Train.KRU["11/3-D1/1"]*Train.KRU["14/1-B3"]*S["B3"])
	S["1-7R-31"] = S["B3"]*Train.KRU["14/1-B3"]*Train.KRP.Value
	S["11B"] = S["10/4"]*Train.KV["10/4-C3"]*(1.0-Train.NR.Value)+T[17]*1
	S["25"] = S["U2"]*Train.KV["U2-25"]*Train.K25.Value
	S["32V"] = S["D1"]*S["D1-32V"]
	S["22E'"] = S["10/4"]*Train.VMK.Value*Train.AK.Value
	S["31V"] = S["D1"]*S["D1-31V"]
...
function TRAIN_SYSTEM.Solve81_714(Train,Triggers)
	...
	S["1Zh-1G"] = Train.LK5.Value*C((P == 1) or (P == 3))*C(RK == 1)*S["1E-1Yu"]+Train.LK3.Value
	S["10"] = 1*Train:ReadTrainWire(10)
	S["2Ye"] = Train.LK4.Value*Train.A2.Value*S["2A-2G"]*(1-2*Train.RRP.Value)*((1-Train.RRP.Value) + Train.RRP.Value*Train.A39.Value)*S["2Zh-2A"]*Train:ReadTrainWire(2)+T[27]*(1.0-Train.LK4.Value)
	S["16V"] = Train.A16.Value*(1.0-Train.RD.Value)*Train:ReadTrainWire(16)
	S["4B"] = (1.0-Train.RKR.Value)*Train:ReadTrainWire(4)
	S["5B"] = Train.RKR.Value*Train:ReadTrainWire(5)
